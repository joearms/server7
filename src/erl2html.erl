...
refresh() ->
    ...
    #{outdir := O} = K,
    O1 = elib2_misc:expand_env_vars(O),
    {ok, [Files]} = file:consult(O1 ++ "all_erl"),
    Files1 = [{filename:basename(I,"*.erl"), I} || I <- Files],
    Files2 = elib2_misc:merge_kv(Files1),
    Files3 = lists:sort(Files2),
...
refresh(In, OutDir) ->
    Mod = filename:basename(In, ".erl"),
    OutFile = OutDir ++ Mod ++ ".html",
    case elib2_misc:out_of_date(In, OutFile) of
	true ->
	    io:format("~nRedo:~p~n",[Mod]),
	    erl2html(In, OutFile);
	false ->
	    io:format("+")
    end.
...
make_file_list() ->
    {ok, [K]} = file:consult("./erl2html.conf"),
    #{outdir := O, erldirs := D} = K,
    O1 = elib2_misc:expand_env_vars(O),
    elib2_misc:glob_dir(O1),
    Dirs = [elib2_misc:expand_env_vars(I) || I <- D],
    %% find all files
    L1 = [elib2_find:files(Dir, "*.erl", true) || Dir <- Dirs],
    L2 = lists:append(L1),
    L3 = lists:sort(L2),
    io:format("found:~w modules~n", [length(L3)]),
    elib2_misc:dump(O1 ++ "all_erl", L3).

erl2html(In, Out) ->
    %% In = .erl file
    %% Out = .html file
    io:format("transforming:~p~n",[In]),
    case (catch erl2html0(In, Out)) of
	{'EXIT', Why} ->
	    io:format("**** Failed for:~p~n reason:~p~n",
		      [In, Why]);
	_ ->
	    void
    end.
...
to_html({dot,_})             -> ".\n";
to_html({callRemote,M,F,A})  -> ["<a href='",M,".html#",F,"/",i2s(A),"'>",F,"</a>"];
