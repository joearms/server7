...
test() ->
    {_,Str} = test("ehe_tests.ehe"),
    file:write_file("ehe_tests.html",Str),
    io:format("written ehe_tests.html~n"),
    true.
...
parse_string(Str) ->
    ...
    case (catch ehe_parser:parse(Toks1)) of
	{ok, Parse} ->
	    trace({parse,ok}),
	    {ok, Parse};
	{error, {Ln,Mod,X}} ->
...
parse_erl_toks([{atom,Ln,else},{dot,_}])   -> {elseToken, Ln};
